---
title: "Prodej bytu 3+kk, 74 m², Vyškov – Sídliště Osvobození"
subtitle: "Světlý byt po rekonstrukci s lodžií a sklepem"
status: aktivni
nabidka: prodej
typ: byt
cena: 4890000
cenaPoznamka: "včetně provize a právního servisu"
lokalita:
  obec: Vyškov
  castObce: Sídliště Osvobození
  okres: Vyškov
  kraj: Jihomoravský kraj
  lat: 49.2775
  lng: 16.9989
dispozice: "3+kk"
plocha: 74
podlazi: 3
pocetPodlazi: 4
stav: "po kompletní rekonstrukci (2021)"
vlastnictvi: osobni
konstrukce: "cihlová"
energetickaTrida: C
vybaveni: ["lodžie", "sklep", "výtah", "parkování před domem", "kuchyňská linka se spotřebiči"]
parametry:
  - { label: "Topení", value: "dálkové" }
  - { label: "Voda", value: "veřejný vodovod" }
  - { label: "Měsíční náklady", value: "cca 4 200 Kč (fond oprav + zálohy)" }
obrazky: ["./01.webp", "./02.webp", "./03.webp", "./04.webp"]
obrazkyAlt: ["Obývací pokoj s okny na západ", "Kuchyňská linka se spotřebiči", "Koupelna po rekonstrukci", "Lodžie s výhledem do vnitrobloku"]
# ukázkové odkazy – ukazují, jak vypadá videoprohlídka a 3D prohlídka na detailu
video: "https://www.youtube.com/watch?v=aqz-KE-bpKQ"
prohlidka3d: "https://my.matterport.com/show/?m=SxQL3iGyoDo"
datum: 2026-08-28
featured: true
externiId: "RA-2026-014"
---
Byt, který nepotřebuje žádnou další investici. V roce 2021 prošel kompletní rekonstrukcí – nové rozvody, podlahy, koupelna i kuchyňská linka. Dispozice 3+kk s obývacím pokojem 28 m² orientovaným na západ, takže odpoledne a večer máte slunce v hlavní místnosti.

## Co je dobré vědět

Dům je cihlový, po revitalizaci (zateplení, nová okna, výtah). Fond oprav je zdravý a bez plánovaných mimořádných příspěvků – výpis z účtu SVJ máme k dispozici. Parkování před domem je bez problémů i večer, škola a školka do 5 minut pěšky, vlakové nádraží 10 minut.

## Proč tato cena

Cena vychází z realizovaných prodejů srovnatelných bytů ve Vyškově za posledních 12 měsíců, ne z nabídkových cen v inzerci. Rádi vám ukážeme, z čeho jsme vycházeli.
